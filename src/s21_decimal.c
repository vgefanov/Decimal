...
big_decimal simple_add(big_decimal op1, big_decimal op2) {
  ...
  unsigned long long accum = 0;
  for (int i = 0; i < 6; i++) {
    accum = accum + (unsigned long long)op1.bits[i] +
            (unsigned long long)op2.bits[i];
    result.bits[i] = (unsigned)accum;
    accum = accum >> 32;
  }
  return result;
...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  big_decimal dst_big_decimal =
      simple_div(value_1_big_decimal, value_2_big_decimal);
  ...
  while (cexp < 28 && big_decimal_is_null(value_1_big_decimal)) {
    ...
    dst_big_decimal = simple_add(
        dst_big_decimal, simple_div(value_1_big_decimal, value_2_big_decimal));
    value_1_big_decimal = simple_mod(value_1_big_decimal, value_2_big_decimal);
    cexp++;
  }
...
int s21_is_less(s21_decimal op1, s21_decimal op2) {
  ...
  int result = FALSE;
  if ((compare_sign_big_decimal(b_op1, b_op2) == 2 &&
       compare_digit_big_decimal(b_op1, b_op2) == -1) ||
      (compare_sign_big_decimal(b_op1, b_op2) == -2 &&
       compare_digit_big_decimal(b_op1, b_op2) == 1) ||
      (!is_null_big_decimal(b_op1) &&
       compare_sign_big_decimal(b_op1, b_op2) == -1))
    result = TRUE;
  return result;
}
...
int s21_is_equal(s21_decimal op1, s21_decimal op2) {
  ...
  if (is_null_big_decimal(b_op1) && is_null_big_decimal(b_op2)) {
    ...
    if ((sign == -2 || sign == 2) &&
        (compare_digit_big_decimal(b_op1, b_op2) == 0)) {
      result = TRUE;
    }
  }
  return result;
